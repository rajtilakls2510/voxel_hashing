...
struct CudaError {
    ...
    std::string message() const {
        return std::string(cudaGetErrorString(code)) + " (" + function + ") at " + file + ":" +
               std::to_string(line);
    }
};
...
inline void cudaCheck(cudaError_t result, const char* func, const char* file, int line) {
    if (result != cudaSuccess) throw CudaError{result, func, file, line};
}
...
public:
    ...
    // options.
    static std::shared_ptr<CudaStream> createCudaStream(CudaStreamType stream_type);
