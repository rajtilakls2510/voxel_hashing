...
public:
    ...
    static constexpr size_t kNumVoxels = kVoxelsPerSide * kVoxelsPerSide * kVoxelsPerSide;

    Block(MemoryType type);
    void clear(const CudaStream& stream = CudaStreamOwning()) override;

    _VoxelType getVoxel(
            const Index3D& index, const CudaStream& cuda_stream = CudaStreamOwning()) const;
    void setVoxel(
            const Index3D& index,
            const _VoxelType value,
            const CudaStream& cuda_stream = CudaStreamOwning());

    static std::shared_ptr<Block<_VoxelType>> copyFrom(
            const Block<_VoxelType>& src,
