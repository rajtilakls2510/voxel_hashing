...
enum class MemoryType { kHost, kDevice, kUnified };
...
// Note vector<Index3D> and vector<Block pointer> modification is removed. Modification of the
// content of each block is not removed.
template <typename BlockType>
using ConstIndexBlockPairs =
        std::pair<const std::vector<Index3D>, const std::vector<typename BlockType::Ptr>>;
