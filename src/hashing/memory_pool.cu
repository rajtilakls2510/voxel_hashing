...
BlockMemoryPool<BlockType>::BlockMemoryPool(size_t min_allocated_blocks,
                                            size_t max_allocated_blocks,
                                            MemoryType type)
    : min_allocated_blocks_(min_allocated_blocks),
      max_allocated_blocks_(max_allocated_blocks), type_(type) {

  if (min_allocated_blocks_ > 0 &&
      max_allocated_blocks >= min_allocated_blocks) {
    ensureCapacity();
  }
}
...
void BlockMemoryPool<BlockType>::ensureCapacity() {
  ...
  if (current_size < min__) {
    ...
    for (size_t i = 0; i < num_blocks_to_allocate; i++) {
      blocks_.push(std::make_unique<BlockType>(type_));
    }
  }
...
typename BlockType::Ptr BlockMemoryPool<BlockType>::popBlock() {
  ensureCapacity();
  typename BlockType::Ptr b = std::move(blocks_.top());
  blocks_.pop();
  return b;
}
...
template <typename BlockType> size_t BlockMemoryPool<BlockType>::size() const {
  return blocks_.size();
}

template <typename BlockType> BlockMemoryPool<BlockType>::~BlockMemoryPool() {}
