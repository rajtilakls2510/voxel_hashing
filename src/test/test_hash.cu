...
int main(int argc, char *argv[])
{
  ...
  IndexBlockPair<FloatBlock> p = {Index3D(0, 1, 0),
                                  std::make_shared<FloatBlock>(host_type)};

  std::vector<Index3D> v = {Index3D(0, 0, 1), Index3D(1, 0, 1)};
  std::vector<FloatBlock::Ptr> b = {std::make_shared<FloatBlock>(host_type),
                                    std::make_shared<FloatBlock>(device_type)};
