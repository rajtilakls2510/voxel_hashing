...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Num blocks: " << layer.numBlocks() << " Num voxels: " << layer.numVoxels()
              << " Voxel size: " << layer.voxel_size() << "\n";
    ...
    // ===========
    {
        ...
        Vector<TsdfVoxel>::Ptr voxels_to_store_ = Vector<TsdfVoxel>::copyFrom(
                voxels_to_store, MemoryType::kHost, stream);  // Doesn't matter where we store
        ...
        Vector<TsdfVoxel>::Ptr voxels_host =
                Vector<TsdfVoxel>::copyFrom(voxels, MemoryType::kHost, stream);
    ...
    // ========
    {
        ...
        TsdfVoxel* retrieved_voxel =
                layer.getVoxelPtr(block_idx_to_retrieve, voxel_idx_to_retrieve, stream);
        TsdfVoxel voxel_on_host(0, 0);
        checkCudaErrors(cudaMemcpyAsync(
                &voxel_on_host, retrieved_voxel, sizeof(TsdfVoxel), cudaMemcpyDefault, stream));
        stream.synchronize();
...

    {
        ...
        if (p.memory_type == MemoryType::kHost) {
            for (size_t i = 0; i < retrieved_voxels.size(); i++) retrieved_voxels[i]->weight += 1.0;
        } else {
            constexpr int kNumThreads = 512;
            const int kNumBlocks = voxel_indices.size() / kNumThreads + 1;
            addWeightKernel<<<kNumBlocks, kNumThreads, 0, stream>>>(
                    retrieved_voxels.size(), retrieved_voxels.data());
            stream.synchronize();
