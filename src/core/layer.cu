...
BlockLayer<BlockType>::BlockLayer(const BlockLayerParams& params)
    : block_size_(params.block_size),
      pool_(params.min_allocated_blocks, params.max_allocated_blocks, params.memory_type),
      type_(params.memory_type) {}
...
template <typename BlockType>
ConstIndexBlockPairs<BlockType> BlockLayer<BlockType>::getBlocks(
        const std::vector<Index3D>& indices) const {
...
std::vector<bool> BlockLayer<BlockType>::allocateBlocks(const std::vector<Index3D>& indices) {
    ...
    for (const auto& index : indices) able_to_allocate.push_back(allocateBlock(index));
...
template <typename BlockType>
std::vector<bool> BlockLayer<BlockType>::areBlocksAllocated(
        const std::vector<Index3D>& indices) const {
    std::vector<bool> found;
    found.reserve(indices.size());
    for (const auto& index : indices) found.push_back(isBlockAllocated(index));
    return found;
}
...
bool BlockLayer<BlockType>::storeBlock(const IndexBlockPair<BlockType>& index_block_pair) {
    ...
    if (!allocation_success) return false;
    ...
    if (!layer_block_ptr) return false;
...
template <typename BlockType>
std::vector<bool> BlockLayer<BlockType>::storeBlocks(
        const ConstIndexBlockPairs<BlockType>& index_block_pairs) {
...
bool BlockLayer<BlockType>::deAllocateBlock(const Index3D& index) {
    ...
    if (it->second) deallocated = pool_.pushBlock(it->second);
...
std::vector<bool> BlockLayer<BlockType>::deAllocateBlocks(const std::vector<Index3D>& indices) {
    ...
    for (const auto& index : indices) deallocated.push_back(deAllocateBlock(index));
    return deallocated;
}

template <typename BlockType>
VoxelBlockLayer<BlockType>::VoxelBlockLayer(const BlockLayerParams& params)
    : BlockLayer<BlockType>(params) {}

template <typename BlockType>
typename BlockType::VoxelType VoxelBlockLayer<BlockType>::getVoxel(
        const Index3D& block_idx, const Index3D& voxel_idx, const CudaStream& stream) const {
    typename BlockType::VoxelType v;
    IndexBlockPair<BlockType> index_block_pair = this->getBlock(block_idx);
    typename BlockType::Ptr block_ptr = index_block_pair.second;
    if (block_ptr) v = block_ptr->getVoxel(voxel_idx, stream);
    return v;
}

template <typename BlockType>
typename BlockType::VoxelType VoxelBlockLayer<BlockType>::getVoxel(
        const Vector3f& positionL, const CudaStream& stream) const {
...
template <typename BlockType>
bool VoxelBlockLayer<BlockType>::storeVoxel(
        const Index3D& block_idx,
        const Index3D& voxel_idx,
        const typename BlockType::VoxelType voxel,
...
template <typename BlockType>
__global__ void getVoxelsKernel(
        size_t num_voxels,
        typename BlockType::VoxelType** block_ptrs,
        Index3D* voxel_indices,
        typename BlockType::VoxelType* voxels) {
...
template <typename BlockType>
Vector<typename BlockType::VoxelType> VoxelBlockLayer<BlockType>::getVoxelsInternal(
        const Vector<Index3D>& block_indices,
        const Vector<Index3D>& voxel_indices,
    ...
    if (this->type_ == MemoryType::kHost) {
        ...
        typename Vector<typename BlockType::VoxelType*>::Ptr block_ptrs =
                Vector<typename BlockType::VoxelType*>::copyFrom(
                        block_ptrs_host, this->type_, stream);
...
template <typename BlockType>
Vector<typename BlockType::VoxelType> VoxelBlockLayer<BlockType>::getVoxels(
        IndexPairs& index_pairs, const CudaStream& stream) const {
...
template <typename BlockType>
Vector<typename BlockType::VoxelType> VoxelBlockLayer<BlockType>::getVoxels(
        const std::vector<Vector3f>& positionsL, const CudaStream& stream) const {
    Vector<Vector3f>::Ptr p_L = Vector<Vector3f>::copyFrom(positionsL, this->type_, stream);
    auto [block_indices, voxel_indices] =
            getBlockAndVoxelIndicesFromPositions(this->block_size_, *p_L, stream);
...
template <typename BlockType>
__global__ void storeVoxelsKernel(
        size_t num_voxels,
        typename BlockType::VoxelType** block_ptrs,
        Index3D* voxel_indices,
        typename BlockType::VoxelType* voxels,
...
template <typename BlockType>
Vector<Bool> VoxelBlockLayer<BlockType>::storeVoxelsInternal(
        const Vector<Index3D>& block_indices,
        const Vector<Index3D>& voxel_indices,
        const Vector<typename BlockType::VoxelType>& voxels,
        const CudaStream& stream) {
    ...
    typename Vector<typename BlockType::VoxelType>::Ptr voxels_to_layer =
            Vector<typename BlockType::VoxelType>::copyFrom(voxels, this->type_, stream);
    ...
    if (this->type_ == MemoryType::kHost) {
        ...
        constexpr int kNumThreads = 512;
        const int kNumBlocks = voxel_indices.size() / kNumThreads + 1;
        storeVoxelsKernel<BlockType><<<kNumBlocks, kNumThreads, 0, stream>>>(
                voxel_indices.size(),
                block_ptrs->data(),
                vi->data(),
...
template <typename BlockType>
Vector<Bool> VoxelBlockLayer<BlockType>::storeVoxels(
...
template <typename BlockType>
Vector<Bool> VoxelBlockLayer<BlockType>::storeVoxels(
        const std::vector<Vector3f>& positionsL,
        const Vector<typename BlockType::VoxelType>& voxels,
        const CudaStream& stream) {
    Vector<Vector3f>::Ptr p_L = Vector<Vector3f>::copyFrom(positionsL, this->type_, stream);
    auto [block_indices, voxel_indices] =
...
template <typename BlockType>
typename BlockType::VoxelType* VoxelBlockLayer<BlockType>::getVoxelPtr(
        const Index3D& block_idx, const Index3D& voxel_idx, const CudaStream& stream) const {
    typename BlockType::VoxelType* v = nullptr;
    IndexBlockPair<BlockType> index_block_pair = this->getBlock(block_idx);
    typename BlockType::Ptr block_ptr = index_block_pair.second;
    if (block_ptr) v = block_ptr->data() + BlockType::idx(voxel_idx);
    return v;
}
...
template <typename BlockType>
Vector<typename BlockType::VoxelType*> VoxelBlockLayer<BlockType>::getVoxelsPtrInternal(
        const Vector<Index3D>& block_indices,
        const Vector<Index3D>& voxel_indices,
        const CudaStream& stream) const {
    Vector<typename BlockType::VoxelType*> block_ptrs_host(block_indices.size(), MemoryType::kHost);
    ...
    return std::move(*(Vector<typename BlockType::VoxelType*>::copyFrom(
            block_ptrs_host, this->type_, stream)));
}

template <typename BlockType>
Vector<typename BlockType::VoxelType*> VoxelBlockLayer<BlockType>::getVoxelsPtr(
        IndexPairs& index_pairs, const CudaStream& stream) const {
    const std::vector<Index3D>& block_indices = index_pairs.first;
    const std::vector<Index3D>& voxel_indices = index_pairs.second;
